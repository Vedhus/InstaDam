...
void InstaDam::openFile_and_labels()
{
    ...
    SelectItem::myBounds = ui->IdmPhotoViewer->setPhotoFromFile(filename, labelPaths);
    qInfo("my bounds set");
#endif
...
void InstaDam::processPointClicked(viewerTypes type, SelectItem *item, QPointF pos, const Qt::MouseButton button){
    ...
    if(!item){
        ...
        switch(currentSelectType){
            ...
            {
                if(drawing){
                    ui->IdmPhotoViewer->setBrush(currentBrushSize, brushMode, currentLabel);
                    ...
                    ui->IdmPhotoViewer->setEraser(currentBrushSize, brushMode, currentLabel);
