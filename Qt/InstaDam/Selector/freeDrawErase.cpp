...
/*!
  ...
  \endlist
  */
FreeDrawErase::FreeDrawErase(QPointF point, int brushSize, Qt::PenCapStyle brushMode, QSharedPointer<Label> label, QGraphicsItem *item)
    ...
    while(it.hasNext()){
        it.next();
        QSharedPointer<QPixmap> delHash = QSharedPointer<QPixmap>::create(SelectItem::myBounds);
